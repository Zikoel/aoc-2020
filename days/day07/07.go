...
func howMutchColorBagCanContain(bag bag, color string) int {
	if bag.color == color {
		return 1
	}
	result := 0
	for _, i := range bag.inner {
		result += i.qt * howMutchColorBagCanContain(*i.bag, color)
	}
	return result
}
...
func Run(data []byte) {
	...
	howManyCanContainShiny := 0
	contained := "shiny gold"
	for _, rule := range rules {

		if rule.bagColorCode == contained {
			continue
		}

		if howMutchColorBagCanContain(*bags[rule.bagColorCode], contained) > 0 {
			howManyCanContainShiny++
		}
	}

	fmt.Printf("%d colors can contain %s", howManyCanContainShiny, contained)
}
